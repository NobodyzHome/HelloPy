# 现在有个需求，需要处理三个数据，并依次对这三个数据做复杂的且耗时较长的逻辑运算(在这里用乘法代替，实际中有可能是调接口、查数据库等非常耗时的代码)。
# 一种实现方式是像下面这样，在函数中依次对这三个数据进行运算，然后一次性将计算结果返回去。
# 这样带来的问题是调用这个函数需要花费大量时间，因为需要一次性串联执行三个复杂的业务逻辑。
...
# 调用包含生成器的函数，此时不会执行gen1函数里的任何代码，而是创建一个生成器对象。因此执行完这行代码不会有任何输出内容。
# 注意：一个生成器对象，每次只能调用next()往前走，无法往回执行代码。
gen1Obj = gen1()
# 第一次执行next()，开始执行gen1函数里的代码，直到执行完第一次遇到yield的代码(yield a * 3)，就停止执行后续代码并返回处理结果了。
# 输出内容：
# 这是一个生成器函数
# 开始处理第一个数据
a1 = next(gen1Obj)
...
# 第二次执行next()，从上一次停止的代码之后（也就是yield a*3之后）继续执行代码，直到执行完下一次yield的代码(yield b * 7)，就停止执行后续代码并返回执行结果。
# 输出内容：开始处理第二个数据
b1 = next(gen1Obj)
# 输出内容：b1=35
print(f'{b1=}')

# 第三次执行next()，从上一次停止的代码之后（也就是yield b*7之后）继续执行代码，直到执行完下一次yield的代码(yield c * c)，就停止执行后续代码并返回执行结果。
# 输出内容：开始处理第三个数据
c1 = next(gen1Obj)
# 输出内容：c1=900
print(f'{c1=}')
...
# 上面那个函数中，写了调用三次yield，所以生成的生成器对象就只能执行三次next。执行第四次时就会报错。
...
# 在实际开发生成器函数时，往往不会像上面那样写出一行行yield语句，而是通过将yield写入到循环语句中，来实现在函数中多次执行到yield语句的效果。
def gen2(x):
...
# 生成器对象什么时候不能被再执行next了
# 1.在上次执行完yield语句的后面，无法再执行到yield语句了，就不能再被执行next了
# 2.再执行一次next就执行到生成器函数中的return的情况
def gen3(x):
...
gen3_obj = gen3(5)
# 第一次执行时i=5，i>2，能执行到yield语句，所以可以执行
y1 = next(gen3_obj)
# 输出内容：y1=25
print(f'{y1=}')
# 第二次执行时i=4，i>2，能执行到yield语句，所以可以执行
y2 = next(gen3_obj)
# 输出内容：y2=16
print(f'{y2=}')
# 第三次执行时i=3，i>2，能执行到yield语句，所以可以执行
y3 = next(gen3_obj)
# 输出内容：y3=9
print(f'{y3=}')
# 第四次执行时i=2，不能满足i>2，直接执行到生成器函数中的return了，所以就不可以被执行了，会报错python-BaseException: StopIteration。
y4 = next(gen3_obj)
print(f'{y4=}')